...
from utils.test_utils import test_model_with_dp, test_model_uncond, test_model_unseen, test_model_multivariate
...
if __name__ == "__main__":
    ...
    if opt.train:
        try:
            trainer.logger.experiment.config.update(vars(opt))
    ...
    if not opt.no_test and not trainer.interrupted:
        if opt.multivariate:
            test_model_multivariate(model, data, trainer, opt, logdir)
        elif opt.uncond:
